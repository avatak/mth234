...
\question Sketch \(\br(t)=\gen{t,t^2}\) in  \(\R^2\) and express the curve as a function of  \(x\) and \(y\). 
        \ifprintanswers
            \begin{solution}
                ...
                \begin{tikzpicture}
                    \begin{axis}[xmin=-3,xmax=3,ymin=-1,ymax=9]
                        \addplot[->,domain=-2.5:2.5] ({\x},{(\x)^2}) node[anchor=south east] {$\br(t)$};
                    \end{axis}
                \end{tikzpicture}
            \end{solution}
        \else
...
\question Sketch \(\br(t)=\gen{4,\cos(t),\sin(t)}\) in \(\R^3\) and express the curve as a function of \(x\), \(y\), and \(z\).
        \ifprintanswers
            \begin{solution}
                ...
                \begin{center}
                \begin{tikzpicture}
                    \draw[->] (0,0)--(0,3) node[anchor=west] {$z$};
                    \draw[->] (0,0)--(3,0) node[anchor=west] {$y$};
                    \draw[->] (0,0)--({4*cos(225)},{4*sin(225)}) node[anchor=south east] {$x$};
                    \draw ($ ({2*cos(225)},{2*sin(225)}) + (0,.1) $) -- ($ ({2*cos(225)},{2*sin(225)}) + (0,-.1) $) node[below] {$4$};
                    \draw[blue] ({2*cos(225)},{2*sin(225)}) circle[radius=1];
                \end{tikzpicture}
                \end{center}
            \end{solution}
        \else
